...
- **Duplicate-lead prevention**: at most one live lead per (job, profile) pair — `apply_job_profile()` soft-deletes any prior live lead before inserting the fresh snapshot (function-enforced, in one transaction).
