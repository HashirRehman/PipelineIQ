...
- **Duplicate-lead prevention**: at most one live lead per (job, profile) pair. Leads are created from the Applied Jobs page via POST `/api/leads`, which enforces the rule (idempotent — returns the existing live lead). Don't add a second creation path that bypasses it.
