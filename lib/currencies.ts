/**
 * ISO 4217 currency list from ksafranski's gist
 * (https://gist.github.com/ksafranski/2973986) — 119 currencies with their
 * three-letter codes, names, and symbols. Single source of truth for the
 * currency dropdown in profile create/edit (the rate-currency field).
 * Kept faithful to the gist, including its older entries (EEK, LTL, LVL,
 * HRK, VEF, ZMK, ZWL…).
 */
...
export const CURRENCIES: readonly Currency[] = [
  ...
  { code: "EEK", name: "Estonian Kroon", symbol: "Ekr" },
...
/** code → currency (e.g. "USD" → US Dollar). */
export const CURRENCY_BY_CODE: ReadonlyMap<string, Currency> = new Map(
  CURRENCIES.map((c) => [c.code, c]),
);

/** True when the string is a known currency code (case-insensitive). */
export function isCurrencyCode(value: string): boolean {
  return CURRENCY_BY_CODE.has(value.trim().toUpperCase());
}
